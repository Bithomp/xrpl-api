...
import { Wallet, Transaction, ValidationError, validate, hashes } from "xrpl";
const { hashSignedTx } = hashes;
import { isValidXAddress, xAddressToClassicAddress } from "ripple-address-codec";
import { encodeForSigning, encodeForMultisigning, encode } from "ripple-binary-codec";
...
export function singTransaction(
  ...
  if (multisignAddress) {
    const signer = {
      ...
      TxnSignature: computeSignature(txToSignAndEncode, wallet.privateKey, multisignAddress),
    };
    txToSignAndEncode.Signers = [{ Signer: signer }];
  } else {
    txToSignAndEncode.TxnSignature = computeSignature(txToSignAndEncode, wallet.privateKey);
  }
 ...
 */
function computeSignature(tx: Transaction, privateKey: string, signAs?: string): string {
  if (signAs) {
